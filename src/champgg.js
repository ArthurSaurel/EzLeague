...
function exctractPage(html, champion, arole, rec, callback, pageType) {
	...
	$("div[class*='Description__Title']", slots).each(function(index) {
		if(index % 11 == 0) {
			pages.push({
				"name": $(".champion-profile h1").text() + "/" + arole + (Math.floor(runecount / 9) ? " [HW]" : " [MF]"),
				"primaryStyleId": -1,
...
function _getPages(champion, aRole, callback) {

	var champUrl = url + "/champion/" + champion + "/" + aRole;
	
	//console.log(champUrl)
	request.get(champUrl, (error, response, html) => {
		if(!error && response.statusCode == 200) {
			exctractPage(html, champion, aRole, true, (pages) => {
				callback(pages, champUrl);
			});
		}
		else {
			callback(error);
